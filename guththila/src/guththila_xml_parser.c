...
GUTHTHILA_EXPORT int GUTHTHILA_CALL guththila_init(guththila_t *m, void *reader)
{
	...
	if (m->reader->type == GUTHTHILA_MEMORY_READER) {
		guththila_buffer_init_for_buffer(&m->buffer, m->reader->buff, m->reader->buff_size);
	} else if (m->reader->type == GUTHTHILA_FILE_READER || GUTHTHILA_IO_READER) {
		guththila_buffer_init(&m->buffer, 0);
	}
...
GUTHTHILA_EXPORT void GUTHTHILA_CALL guththila_free(guththila_t *m)
{
	...
	guththila_buffer_un_init(&m->buffer);
	free(m);
	return;
}
...
GUTHTHILA_EXPORT int GUTHTHILA_CALL guththila_next(guththila_t *m)
{
	...
	if ('<' == c && m->status == S_2) {	
		...
		if (c != '?' && c != '!' && c != '/'){
			...
			if (2 == guththila_next_no_char(m, 0, c_arra, 2) && '-' == c_arra[0] && '-' == c_arra[1]){
				c = guththila_next_char(m, 0);
				GUTHTHILA_TOKEN_OPEN(m, tok);
				while (1) {
					c = guththila_next_char(m, 0);
...
static int GUTHTHILA_CALL guththila_process_xml_dec(guththila_t *m)
{
	...
	if (3 == guththila_next_no_char(m, GUTHTHILA_EOF, c_arra, 3) && 'x' == c_arra[0] && 'm' == c_arra[1] && 'l' == c_arra[2]) {
		c = guththila_next_char(m, GUTHTHILA_EOF);
		GUTHTHILA_SKIP_SPACES(m, c);
		
		if (c == 'v') {
			GUTHTHILA_TOKEN_OPEN(m, tok); 
			if(6 == guththila_next_no_char(m, 0, c_arra, 6) && 
		...
		if (c == 'e') {
			GUTHTHILA_TOKEN_OPEN(m, tok);
			if(7 == guththila_next_no_char(m, 0, c_arra, 7) &&
		...
		if (c == 's') {
			GUTHTHILA_TOKEN_OPEN(m, tok);			
			if(9 == guththila_next_no_char(m, 0, c_arra, 9) &&
	...
	if (m->reader->type == GUTHTHILA_MEMORY_READER && m->next < GUTHTHILA_BUFFER_CURRENT_DATA_SIZE(m->buffer)){        		
		...
		if (m->next < GUTHTHILA_BUFFER_PRE_DATA_SIZE(m->buffer) + GUTHTHILA_BUFFER_CURRENT_DATA_SIZE(m->buffer) && m->buffer.cur_buff != -1) {
			...
			if (m->buffer.cur_buff == m->buffer.no_buffers - 1) {				
				...
				free(m->buffer.buff);
				free(m->buffer.data_size);
				free(m->buffer.buffs_size);
				m->buffer.buff = temp1;
			...
			m->buffer.cur_buff++;
			m->buffer.buff[m->buffer.cur_buff] = (char *)malloc(sizeof(char) * m->buffer.buffs_size[m->buffer.cur_buff - 1] * 2);    
			...
			temp = guththila_reader_read(m->reader, GUTHTHILA_BUFFER_CURRENT_BUFF(m->buffer), 0, GUTHTHILA_BUFFER_CURRENT_BUFF_SIZE(m->buffer));			
			if (temp > 0) {
			...
			m->buffer.buff[0] = (char *) malloc(sizeof(char) * GUTHTHILA_BUFFER_DEF_SIZE);
			m->buffer.buffs_size[0] = GUTHTHILA_BUFFER_DEF_SIZE;
			m->buffer.cur_buff = 0;
			temp = guththila_reader_read(m->reader, m->buffer.buff[0], 0, GUTHTHILA_BUFFER_DEF_SIZE);
...
static int GUTHTHILA_CALL guththila_next_no_char (guththila_t *m, int eof, char *bytes, int no)
{
	...
	if (m->reader->type == GUTHTHILA_MEMORY_READER && m->next + no - 1 < GUTHTHILA_BUFFER_CURRENT_DATA_SIZE(m->buffer) && m->buffer.cur_buff != -1){        
		...
		if (m->next < GUTHTHILA_BUFFER_PRE_DATA_SIZE(m->buffer) + GUTHTHILA_BUFFER_CURRENT_DATA_SIZE(m->buffer) + no && m->buffer.cur_buff != -1) {
			...
			m->buffer.buff[m->buffer.cur_buff + 1] = (char *)malloc(sizeof(char) * m->buffer.data_size[m->buffer.cur_buff] * 2);    
			m->buffer.cur_buff++;
			m->buffer.buffs_size[m->buffer.cur_buff] = m->buffer.buffs_size[m->buffer.cur_buff - 1] * 2;
			m->buffer.data_size[m->buffer.cur_buff] = 0;						
