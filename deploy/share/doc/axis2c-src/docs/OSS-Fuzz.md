...
| `fuzz_json_reader.c` | JSON→AXIOM conversion (HTTP/1.1) | Malformed JSON, type confusion |
...
| `fuzz_finbench.c` | Financial Benchmark Service — full request→compute→response | Integer overflow, exp() overflow, div-by-zero, hash keys, array bounds |
