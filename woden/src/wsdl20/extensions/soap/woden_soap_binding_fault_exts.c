 ...
 */
struct woden_soap_binding_fault_exts_impl
{
    ...
    const axutil_env_t *env)
{
    ...
    if (!binding_fault_exts)
    {
        ...
        binding_fault_exts_impl = (woden_soap_binding_fault_exts_impl_t *) binding_fault_exts;
...
create(const axutil_env_t *env)
{
    ...
    binding_fault_exts_impl->obj_type = WODEN_SOAP_BINDING_FAULT_EXTS;
    ...
    binding_fault_exts_impl->binding_fault_exts.ops = AXIS2_MALLOC(env->allocator,
            sizeof(woden_soap_binding_fault_exts_ops_t));

    binding_fault_exts_impl->binding_fault_exts.ops->free = woden_soap_binding_fault_exts_free;
