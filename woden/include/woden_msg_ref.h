...
#ifndef WODEN_MSG_REF_H
#define WODEN_MSG_REF_H

/**
 * @file woden_msg_ref.h
...
/** @defgroup woden_msg_ref Interface Message Reference
  * @ingroup woden
  * @{
  */

typedef union woden_msg_ref_base woden_msg_ref_base_t;
typedef struct woden_msg_ref woden_msg_ref_t;
typedef struct woden_msg_ref_ops woden_msg_ref_ops_t;
...
#define WODEN_MSG_REF_FREE(msg_ref, env) \
      (((woden_msg_ref_t *) msg_ref)->ops->\
       free(msg_ref, env))

#define WODEN_MSG_REF_SUPER_OBJS(msg_ref, env) \
      (((woden_msg_ref_t *) msg_ref)->ops->\
       super_objs(msg_ref, env))

#define WODEN_MSG_REF_TYPE(msg_ref, env) \
      (((woden_msg_ref_t *) msg_ref)->ops->\
       type(msg_ref, env))

#define WODEN_MSG_REF_GET_BASE_IMPL(msg_ref, env) \
      (((woden_msg_ref_t *) msg_ref)->ops->\
       get_base_impl(msg_ref, env))

#define WODEN_MSG_REF_GET_QNAME(msg_ref, env) \
      (((woden_msg_ref_t *) msg_ref)->ops->\
         get_qname(msg_ref, env))

#define WODEN_MSG_REF_TO_ELEMENT(msg_ref, env) \
      (((woden_msg_ref_t *) msg_ref)->ops->\
