...
echo "🚀 Building Login JSON Service - SOAP-FREE HTTP/2 Architecture"

# Create services directory
echo "📁 Creating services directory..."
sudo mkdir -p /usr/local/axis2c/services/LoginService
...
echo "✅ INCLUDING: -laxutil -laxis2_engine -ljson-c -lssl -lcrypto -lm"
...
gcc -shared -fPIC \
    ...
    src/login_service.c \
    src/login_json_handler.c \
    -o liblogin_json_service.so \
    -L/usr/local/axis2c/lib \
    -laxutil -laxis2_engine \
    -ljson-c -lssl -lcrypto -lm

echo "Build exit code: $?"

if [ -f "liblogin_json_service.so" ]; then
    echo ""
    echo "🎉 SUCCESS: Login JSON Service built with SOAP-FREE architecture!"

    # Show file info
    ls -la liblogin_json_service.so
    file liblogin_json_service.so
    ...
    ldd liblogin_json_service.so | grep -E '(axis2|json|libax)' || echo "   (Dependencies shown above in full ldd output)"

    # Install the service
    echo ""
    echo "🚀 Installing SOAP-free JSON service..."
    sudo cp liblogin_json_service.so /usr/local/axis2c/services/LoginService/liblogin_service.so
    sudo cp services.xml /usr/local/axis2c/services/LoginService/
