    ...
    axiom_node_t ** node)
{
    ...
    *node = axiom_node_create(env);
    ...
    if (value)
    {
        processing_instruction->value = (axis2_char_t*)axis2_strdup(env, value);
    ...
    if (target)
    {
        processing_instruction->target = (axis2_char_t*) axis2_strdup(env, target);
    ...
    const axis2_char_t *value)
{
    ...
    om_pi->value = (axis2_char_t*)axis2_strdup(env, value);
    ...
    const axis2_char_t *target)
{
    ...
    om_pi->target = (axis2_char_t*)axis2_strdup(env, target);
    return AXIS2_SUCCESS;
}
