...
{
  XML_PullParser *parser = (XML_PullParser *) malloc (sizeof (XML_PullParser));
...
{
  memmove (parser->buffer->buff, parser->buffer->buff+(parser->offset), (parser->_next) - (parser->offset));
 ...
 int c = 0;
  if (parser->_next == parser->buffer->size)
      ...
      else
	{
	  ...
	  XML_PullParser_relocateTokens (parser, (b->size - parser->buffer->size));
	}
    }
  c = Reader_read ((parser->buffer->buff), (parser->_next), (parser->buffer->size)-(parser->_next), parser->reader);
  parser->last += c;
  return !c;
}
...
XML_PullParser_processName (XML_PullParser *parser)
{
  ...
  XML_PullParser_openToken (parser);
  c = XML_PullParser_nextChar (parser, 0);
  while (!(XML_PullParser_isS (c) || '/' == c || '=' == c || '?' == c || '>' == c))
  {
    if ( ':' == c)
      {
	...
	for (; is > 0 && Stack_size (parser->stack); is--)
	  {
		...
		is --;
		name = e->token;
		if (!Token_compare (e->token, "xmlns", 5, parser->unicode_state))
		...
		if (!Token_compare (e->token, "xmlns", 5, parser->unicode_state))
		  {
		    ...
		    XML_PullParser_addNamespace (parser, ex->attribute->name, ex->attribute->value);
		  }
		else
		  {
		    if (parser->name)
			parser->prefix = e->token;
		      ...
		      {
			ex = Stack_pull (parser->attrib);
			XML_PullParser_addAttribute_with_prefix (parser, e->token, ex->attribute->name, ex->attribute->value); 
		      }
		  }
	      }
	    else
...
PRIVATE void
XML_PullParser_addAttribute (XML_PullParser *parser, TOKEN *name, TOKEN *value)
{
  ATTRIBUTE *att;
...
PRIVATE void
XML_PullParser_addAttribute_with_prefix (XML_PullParser *parser, TOKEN *prefix, TOKEN *name, TOKEN *value)
{
  ATTRIBUTE *att;
  att = (ATTRIBUTE *) malloc (sizeof (ATTRIBUTE));
  att->name = name;
...
PRIVATE void
XML_PullParser_addNamespace (XML_PullParser *parser, TOKEN *name, TOKEN *uri)
{
  NAMESPACE *ns;
