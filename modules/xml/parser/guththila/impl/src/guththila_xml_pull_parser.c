                                       ...
                                       int t, int refer)
{
    guththila_element_t *e =
        guththila_stack_last (environment, parser->stack);
    e->token->type = t;
    e->token->ref = refer;
    e->token->end =
...
int
guththila_xml_pull_parser_process_version_info (guththila_environment_t *
                                                environment,
...
int
guththila_xml_pull_parser_process_encoding_decl (guththila_environment_t *
                                                 environment,
                                                 guththila_xml_pull_parser_t *
                                                 parser)
{
...
int
guththila_xml_pull_parser_process_sd_decl (guththila_environment_t *
                                           environment,
                                           guththila_xml_pull_parser_t *
                                           parser)
{
...
int
guththila_xml_pull_parser_process_xml_decl (guththila_environment_t *
                                            environment,
                                            guththila_xml_pull_parser_t *
                                            parser)
{
...
int
guththila_xml_pull_parser_is_space (guththila_environment_t * environment,
                                    int c)
{
...
int
guththila_xml_pull_parser_is_valid_starting_char (guththila_environment_t *
                                                  environment,
                                                  guththila_xml_pull_parser_t
                                                  * parser, int c)
{
...
int
guththila_xml_pull_parser_process_attribute_value (guththila_environment_t *
                                                   environment,
                                                   guththila_xml_pull_parser_t
                                                   * parser, int quote)
{
...
int
guththila_xml_pull_parser_process_attribute (guththila_environment_t *
                                             environment,
                                             guththila_xml_pull_parser_t *
                                             parser, int c)
{
...
int
guththila_xml_pull_parser_processSTagOrEmptyElem (guththila_environment_t *
                                                  environment,
                                                  guththila_xml_pull_parser_t
                                                  * parser)
{
...
int
guththila_xml_pull_parser_process_char_data (guththila_environment_t *
                                             environment,
                                             guththila_xml_pull_parser_t *
                                             parser)
{
...
int
guththila_xml_pull_parser_process_comment (guththila_environment_t *
                                           environment,
                                           guththila_xml_pull_parser_t *
                                           parser)
{
    ...
    case GUTHTHILA_EMPTY_ELEMENT:
        {
            ...
            for (;
                 is > 0 && guththila_stack_size (environment, parser->stack);
                 is--)
            {
                e = guththila_stack_pull (environment, parser->stack);
                ...
                else if (e->token->type == _prefix)
                {
                    ...
                    if (!guththila_token_compare
                        (environment, e->token, "xmlns", 5,
                         parser->unicode_state))
                    {
                        ex = guththila_stack_pull (environment,
                                                   parser->attrib);
                        guththila_xml_pull_parser_add_namespace (environment,
                                                                 parser,
                                                                 ex->
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            guththila_xml_pull_parser_add_attribute_with_prefix
                                (environment, parser, e->token,
                                 ex->attribute->name, ex->attribute->value);
                        }
                    }
                }
                else
...
void
guththila_xml_pull_parser_close_element (guththila_environment_t *
                                         environment,
                                         guththila_xml_pull_parser_t * parser)
{
    ...
    for (; ii > 0; ii--)
    {
        ...
        if (e->_namespace->name)
        {
            free (e->_namespace->name);
            e->_namespace->name = NULL;
        }
        if (e->_namespace->uri)
        {
            free (e->_namespace->uri);
            e->_namespace->uri = NULL;
        }
        if (e)
        {
            free (e);
...
void
guththila_xml_pull_parser_add_attribute (guththila_environment_t *
                                         environment,
                                         guththila_xml_pull_parser_t * parser,
...
void
guththila_xml_pull_parser_add_attribute_with_prefix (guththila_environment_t *
                                                     environment,
                                                     guththila_xml_pull_parser_t
                                                     * parser,
                                                     guththila_token_t *
...
void
guththila_xml_pull_parser_add_namespace (guththila_environment_t *
                                         environment,
                                         guththila_xml_pull_parser_t * parser,
                                         guththila_token_t * name,
...
(guththila_environment_t * environment, guththila_xml_pull_parser_t * parser, int i)
{
